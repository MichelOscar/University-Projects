...
real*8 genrand_real2,E,energia,mag,magne,temp,x,y,delta,u,ebis,W(-8:8),erre,errm,time1,time2,date
real*8 SUMA,SUME,SUME2,SUMM,SUMM2,SUMAM,vare,varm
...
erre=sqrt((sume2-sume*sume)/suma)/(dfloat(L*L))
errm=sqrt((abs(summ2-sumam*sumam))/suma)/(dfloat(L*L))
...
subroutine config(S,L)
integer L,i,j
integer*2 S(1:L,1:L)
do i=1,L
do j=1,L
if (S(i,j).gt.0.5d0) then
write(12,*)i,j
endif
enddo
enddo
